...
class WxPayApi
{
     ...
     * @var path
     */
    public static $sslcer_path = '../cert/apiclient_cert.pem';
    ...
    {
        self::$appid = isset($this->config['appid']) ? $this->config['appid'] : '';
        self::$mch_id = isset($this->config['mch_id']) ? $this->config['mch_id'] : '';
        self::$key = isset($this->config['key']) ? $this->config['key'] : '';
        self::$app_secret = isset($this->config['app_secret']) ? $this->config['app_secret'] : '';
        if (isset($this->config['sslcer_path'])) {
            self::$sslcer_path = $this->config['sslcer_path'];
        }
        if (isset($this->config['sslkey_path'])) {
            self::$sslkey_path = $this->config['sslkey_path'];
        }
        if (isset($this->config['notify_url'])) {
            self::$sslkey_path = $this->config['notify_url'];
        }
        if (isset($this->config['curl_proxy_host'])) {
            self::$curl_proxy_host = $this->config['curl_proxy_host'];
        }
        if (isset($this->config['curl_proxy_port'])) {
            self::$curl_proxy_port = $this->config['curl_proxy_port'];
        }
        if (isset($this->config['report_level'])) {
            self::$report_level = $this->config['report_level'];
        }
    }
    ...
    public static function unifiedOrder($inputObj, $timeOut = 6)
    {
        ...
        if (!$inputObj->IsNotify_urlSet()) {
            $inputObj->SetNotify_url(self::$notify_url);//异步通知url
        }

        $inputObj->SetAppid(self::$appid);//公众账号ID
        $inputObj->SetMch_id(self::$mch_id);//商户号
        $inputObj->SetSpbill_create_ip($_SERVER['REMOTE_ADDR']);//终端ip
        //$inputObj->SetSpbill_create_ip("1.1.1.1");
        $inputObj->SetNonce_str(self::getNonceStr());//随机字符串

        //签名
        $inputObj->SetSign(self::$key);
        $xml = $inputObj->ToXml();

        $startTimeStamp = self::getMillisecond();//请求开始时间
        $response = self::postXmlCurl($xml, $url, false, $timeOut);
        $result = WxPayResults::Init($response, self::$key);
    ...
    private static function postXmlCurl($xml, $url, $useCert = false, $second = 30)
    {
        ...
        if ($useCert == true) {
            ...
            curl_setopt($ch, CURLOPT_SSLCERT, self::$sslcer_path);
